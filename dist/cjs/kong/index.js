...
const hashMessageEIP191SolidityKeccakNoNonce = (address, hash) => {
    ...
    const message = ethers.ethers.utils.solidityKeccak256(["address", "bytes32"], [address, hash]);
    return ethers.ethers.utils.solidityKeccak256(["string", "bytes32"], [messagePrefix, ethers.ethers.utils.arrayify(message)]);
};
const hashMessageEIP191SolidityKeccak = (address, hash, nonce) => {
    const messagePrefix = "\x19Ethereum Signed Message:\n32";
    const message = address
        ? ethers.ethers.utils.solidityKeccak256(["address", "bytes32", "uint256"], [address, hash, nonce])
        : ethers.ethers.utils.solidityKeccak256(["bytes32"], [hash]);
    return ethers.ethers.utils.solidityKeccak256(["string", "bytes32"], [messagePrefix, ethers.ethers.utils.arrayify(message)]);
};
const unpackDERsignature = (signature) => {
...
function parseKeys(payload) {
    try {
        ...
        const primaryPublicKeyHash = ethers.ethers.utils.sha256("0x" + primaryPublicKeyRaw.slice(2));
        const secondaryPublicKeyLength = parseInt("0x" +
            payload.slice(primaryPublicKeyLength + 2, primaryPublicKeyLength + 4)) * 2;
        const secondaryPublicKeyRaw = payload.slice(primaryPublicKeyLength + 4, primaryPublicKeyLength + secondaryPublicKeyLength + 4);
        const secondaryPublicKeyHash = ethers.ethers.utils.sha256("0x" + secondaryPublicKeyRaw.slice(2));
        const tertiaryPublicKeyLength = parseInt("0x" +
            payload.slice(primaryPublicKeyLength + secondaryPublicKeyLength + 4, primaryPublicKeyLength + secondaryPublicKeyLength + 6)) * 2;
        let tertiaryPublicKeyRaw = null;
        let tertiaryPublicKeyHash = null;
        if (tertiaryPublicKeyLength > 0) {
            tertiaryPublicKeyRaw = payload.slice(primaryPublicKeyLength + secondaryPublicKeyLength + 6, primaryPublicKeyLength +
                secondaryPublicKeyLength +
                tertiaryPublicKeyLength +
                6);
            tertiaryPublicKeyHash = ethers.ethers.utils.sha256("0x" + tertiaryPublicKeyRaw.slice(2));
        }
        const keys = {
...
async function triggerScan({ reqx, rpId }) {
    try {
...
const getSignatureFromScan = async ({ rpId, chipPublicKey, address, hash, nonce, }) => {
    if (!IS_BROWSER) {
    ...
    const computedAddress = ethers.ethers.utils.computeAddress("0x" + chipPublicKey);
...
const getPublicKeysFromScan = async ({ rpId, } = {}) => {
    if (!IS_BROWSER) {
        console.warn("This function is only available in a browser environment.");
        return;
    }
...
exports.generateCmdForRegistration = generateCmdForRegistration;
exports.getPublicKeysFromScan = getPublicKeysFromScan;
exports.getSignatureFromScan = getSignatureFromScan;
